...
[![IMAGE ALT TEXT HERE](https://img.youtube.com/vi/CXMkFQFXQCo/0.jpg)](https://www.youtube.com/watch?v=CXMkFQFXQCo)
 ...
 * [Автоматическая настройка зависимостей](#Автоматическая-настройка-зависимостей)
   * [Генерация на основе параметров конструктора](#Генерация-на-основе-параметров-конструктора)
 ...
 * [Продвинутые примеры использования](#Продвинутые-примеры-использования)
   ...
   * [Интеграция с фреймворком на примере slim framework](#Интеграция-с-фреймворком-на-примере-slim-framework)
...
Рассмотрим пример подключения параметров из JSON файла
...
Для конфигурации можно использовать обычные php файлы с ассоциативный массивом (ключ, значение) который будет доступен из контейнера.

Плюс php файлов в том, что можно использовать константы PHP, а также имена классов ([::class](http://php.net/manual/en/language.oop5.basic.php#language.oop5.basic.class.class)) и автокомлит.
 ...
 * Перед созданием Container нужно создать все объекты и мы получаем паттерн [Registry](https://designpatternsphp.readthedocs.io/ru/latest/Structural/Registry/README.html) вместо загрузки по необходимости.
 * Перед созданием объекта нужно быть уверенным что нужные параметры объявленны
 * Все это будет лежать в одном конфиге и будет адом наяву.
 
Для решения всех этих проблем используется [LoaderInterface](https://github.com/smpl/mydi/blob/master/src/LoaderInterface.php), а конкретней Service смотрите следующий раздел.

### Service

Service - это объект реализующий [LoaderInterface](https://github.com/smpl/mydi/blob/master/src/LoaderInterface.php). Объекты реализующие [LoaderInterface](https://github.com/smpl/mydi/blob/master/src/LoaderInterface.php), Container обрабатывает по другому, 
...
Объявление Alias удобно выносить в отдельный файл, чтобы потом в нем 'регистрировать' новые интерфейсы и абстракные 
...
Реальные применения этой штуки даже не могу придумать, но вдруг кому понадобится.
...
Данный провайдер преобразуем имя запрощенного контейнера в путь до файла PHP и этот файл должен возвращать результат 
или один из загрузчиков (Service, Alias, Factory) описанных выше, если такого файла нет - то этот провайдер ничего не 
загружает.

Давайте создадим для примера объект с параметрами для подключения к бд для этого нам понадобиться:
...
Как мы видим этот провайдер позволяет также вручную конфигурировать объекты, но в отличие от [KeyValue](https://github.com/smpl/mydi/blob/master/src/Provider/KeyValue.php) он не загружает 
...
Провайдер ([Autowire](https://github.com/smpl/mydi/blob/master/src/Provider/Autowire.php)) автоматически определяет зависимости и создает объекты, его лучше подключать в самом конце.

### Генерация на основе параметров конструктора

Самое простое и универсальное средство это писать код указывая в конструкторе зависимости которые вам нужны, с помощью 
типов или имени аргумента.
...
приходит [ContainerAwareInterface](https://github.com/smpl/mydi/blob/master/src/ContainerAwareInterface.php) который доступен внутри провайдера [Autowire](https://github.com/smpl/mydi/blob/master/src/Provider/Autowire.php), давайте посмотрим пример
...
setContainer в который передают [Container](https://github.com/smpl/mydi/blob/master/src/Container.php) который может загрузить остальные части или просто сохранить этот container и 
работать как service locator.

Любителям service locator есть [ContainerAwareTrait](https://github.com/smpl/mydi/blob/master/src/ContainerAwareTrait.php) который будет сохранять container в переменную.
...
### Интеграция с фреймворком на примере slim framework
...
и реализовать ContainerAwareInterface, но зачем так сложно настраивать ?).
...
[Фаулера](https://martinfowler.com/articles/injection.html), но в некоторых случаях когда надо быстро наговнокодить 
могу использовать [ContainerAwareInterface](https://github.com/smpl/mydi/blob/master/src/ContainerAwareInterface.php).
