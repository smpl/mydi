...
#### Service ####
Это почти тоже самое что и Factory, только **callable элемент вызывается ОДИН раз**.

В этот контейнер **БОЛЬШЕ НЕ ОБОРАЧИВАЮТСЯ АНОНИМНЫЕ ФУНКЦИИ** начиная с версии 1.1.* смотри #36
```php
$locator->add('dsn', 'mysql:dbname=testdb;host=127.0.0.1');
$locator->add('user', 'dbuser');
$locator->add('password', 'dbpass');
...
$magic1 = $locator['magic'](1);
$magic2 = $locator['magic'](2);
